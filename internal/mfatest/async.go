// Package mfatest registers a "push" MFA driver verified out-of-band, for tests of the poll
// and webhook flows.
package mfatest
...
// HeaderCallbackSecret carries the shared secret authenticating the webhook.
...
// Open creates the method; config key callback_secret guards the webhook.
func (driver) Open(store mfa.Store, cfg *contract.MFAMethodConfig) (mfa.Method, error) {
	return &method{store: store, name: cfg.Name, secret: cfg.Config["callback_secret"], challenges: make(map[string]*challenge)}, nil
}
...
func (m *method) BeginVerify(ctx context.Context, userID string) (string, map[string]any, error) {
	...
	m.seq++
	m.challenges[id] = &challenge{userID: userID, result: mfa.VerifyPending, seq: m.seq}

	return id, map[string]any{"transaction": id[:6]}, nil
}

// Verify reports the challenge state; approval only ever arrives through HandleCallback or
// Approve.
...
// HandleCallback settles a challenge from the webhook body {"challenge_id":"...","approved":true}.
...
// Approve settles userID's newest pending challenge on m, standing in for the device. It
// reports whether a pending challenge was found.
func Approve(m mfa.Method, userID string, approved bool) bool {
	...
	for _, c := range pm.challenges {
		if c.userID == userID && c.result == mfa.VerifyPending && (newest == nil || c.seq > newest.seq) {
			newest = c
		}
	}
